...
async function search(apiKey, type, id, userConfig = {}) {
    ...
    const { signal, cleanup: cleanupAbort } = debridHelpers.createAbortControllerWithTimeout(SEARCH_TIMEOUT_MS, 'DBA');
    ...
    try {
        ...
        console.log(`[${LOG_PREFIX}] Found ${torrents.length} torrents from scrapers.`);

        // OPTIMIZED: Check cache store FIRST to avoid redundant API calls
        ...
        try {
            if (sqliteCache?.isEnabled()) {
                ...
                console.log(`[${LOG_PREFIX}] Cache returned ${cachedHashesFromSqlite.size}/${hashes.length} cached hashes`);
            }
        } catch (error) {
            console.error(`[${LOG_PREFIX} CACHE] Error getting cached hashes: ${error.message}`);
        }

        // OPTIMIZED: ONLY call Debrider.app API for hashes not in cache store (saves API bandwidth/rate limits)
        const hashesToCheck = hashes.filter(h => !cachedHashesFromSqlite.has(h.toLowerCase()));
        let cachedTorrents = [];

        if (hashesToCheck.length > 0) {
            console.log(`[${LOG_PREFIX}] Cache missing ${hashesToCheck.length} hashes, checking Debrider.app API...`);
            cachedTorrents = await checkCache(apiKey, hashesToCheck);
            console.log(`[${LOG_PREFIX}] Debrider.app API returned ${cachedTorrents.length} cached torrents`);
        } else {
            console.log(`[${LOG_PREFIX}] ⚡ SKIPPED Debrider.app API check (cache has all ${cachedHashesFromSqlite.size} hashes!)`);
        }

        // For hashes found in SQLite but not checked with API, assume they're still cached
        // Note: We won't have file details for these, so they'll be filtered out in the merge step
        console.log(`[${LOG_PREFIX}] Processing ${cachedTorrents.length} cached torrents from API`);
        ...
        const mergedTorrents = cachedTorrents.flatMap(cachedTorrent => {
            ...
            if (videoFiles.length === 1) {
                ...
                return {
                    ...
                    source: 'debriderapp',
                    tracker: [originalTorrent?.tracker, originalTorrent?.Tracker, originalTorrent?.originalSource]
