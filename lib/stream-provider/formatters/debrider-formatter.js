...
import { getResolutionFromName, formatSize, extractFileName } from '../../common/torrent-utils.js';
...
export function toDebriderStream(details, type, config) {
    ...
    // Apply proxy wrapping for DebriderApp/PersonalCloud direct URLs when enabled
    let finalTitle = title;
    if (!details.url.startsWith('nzb:') && Array.isArray(config.DebridServices)) {
        const serviceConfig = config.DebridServices.find(s =>
            s.provider === 'DebriderApp' || s.provider === 'PersonalCloud'
        );

        if (serviceConfig?.enableProxy && serviceConfig?.proxyUrl) {
            const proxyParams = new URLSearchParams();
            proxyParams.set('d', streamUrl);
            if (serviceConfig.proxyPassword) {
                proxyParams.set('api_password', serviceConfig.proxyPassword);
            }
            const proxyBase = serviceConfig.proxyUrl.replace(/\/+$/, '');
            streamUrl = `${proxyBase}/proxy/stream?${proxyParams.toString()}`;
            finalTitle = `${title}\n🔒 Proxy`;
        }
    }
