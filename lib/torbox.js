...
const TIMEOUT = 15000;
const TB_MAX_RETRIES = parseInt(process.env.TB_MAX_RETRIES || '2', 10);
const TB_RETRY_DELAY_MS = parseInt(process.env.TB_RETRY_DELAY_MS || '800', 10);
...
function isRetryableTorboxError(error) {
...
async function torboxRequest(requestFn, label) {
    let lastError;
    for (let attempt = 0; attempt <= TB_MAX_RETRIES; attempt++) {
        try {
            return await requestFn();
        } catch (error) {
            lastError = error;
            if (attempt < TB_MAX_RETRIES && isRetryableTorboxError(error)) {
                const delayMs = Math.min(TB_RETRY_DELAY_MS * (attempt + 1), 5000);
                console.warn(`[${LOG_PREFIX}] ${label} failed, retrying in ${delayMs}ms... (${attempt + 1}/${TB_MAX_RETRIES})`);
                await delay(delayMs);
                continue;
            }
            throw error;
...
async function searchTorboxTorrents(apiKey, type, id, userConfig = {}) {
    ...
    const { signal, cleanup: cleanupAbort } = debridHelpers.createAbortControllerWithTimeout(SEARCH_TIMEOUT_MS, 'TB');
    ...
    try {
        ...
        // Fetch cached scraper results in parallel with live scrapers
        ...
        let [personalFiles, scraperResults, cachedScraperResults] = await Promise.all([
        ...
        if (seriesCtx) {
            scraperResultsArray = scraperResultsArray.map(list => list.filter(t => matchesSeriesCandidate(t)));
...
async function combineAndMarkResults(apiKey, personalFiles, externalSources, episodeInfo = null, cinemetaDetails, seriesCtx = null) {
    // Build local matchesSeriesCandidate using seriesCtx (and short-title variant)
    const seriesCtxShort = (() => {
        if (!seriesCtx || !cinemetaDetails?.name) return null;
        const fullTitle = String(cinemetaDetails.name).trim();
        if (!fullTitle.includes(':')) return null;
        const shortTitle = fullTitle.split(':')[0].trim();
        if (!shortTitle || shortTitle === fullTitle) return null;
        // Reconstruct search string from seriesCtx season/episode for parseSeriesQuery
        const s = seriesCtx.season, e = seriesCtx.episode;
        const searchStr = (Number.isFinite(s) && Number.isFinite(e))
            ? `${shortTitle} s${String(s).padStart(2, '0')}e${String(e).padStart(2, '0')}`
            : shortTitle;
        return buildSeriesContext({ search: searchStr, cinemetaTitle: shortTitle });
    })();
    const matchesSeriesCandidate = (t) => {
        if (!seriesCtx) return true;
        if (matchesCandidateTitle(t, seriesCtx)) return true;
    ...
    if (seriesCtx) {
        episodeFiltered = uniqueExternalTorrents.filter(t => matchesSeriesCandidate(t));
    ...
    if (seriesCtx) {
        franchiseFiltered = validTitleTorrents.filter(t => matchesSeriesCandidate(t));
    }
...
async function checkTorboxCache(apiKey, hashes) {
    ...
    try {
        ...
        const response = await torboxRequest(
            () => axios.post(url, { hashes: hexHashes }, debridProxyManager.getAxiosConfig('torbox', { headers })),
            'cache check'
        );
...
async function getTorrentList(apiKey, bypassCache = false) {
    ...
    try {
        const response = await torboxRequest(
            () => axios.get(url, debridProxyManager.getAxiosConfig('torbox', { headers, params, timeout: TIMEOUT })),
            'torrent list'
        );
...
async function getTorrentInfoFromCache(apiKey, hash) {
    ...
    try {
        const response = await torboxRequest(
            () => axios.post(url, data, debridProxyManager.getAxiosConfig('torbox', { headers, timeout: TIMEOUT })),
...
async function requestDownloadLink(apiKey, torrentId, fileId, clientIp) {
    ...
    const response = await torboxRequest(
        () => axios.get(url, debridProxyManager.getAxiosConfig('torbox', { params, headers, timeout: TIMEOUT })),
