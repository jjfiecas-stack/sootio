...
import { getCachedScraperResults, getCachedHashesForRelease } from './util/cache-store.js';
...
const { isValidVideo, isValidTorrentTitle, getResolutionFromName, resolutionOrder, delay, filterByYear, normalizeInfoHash } = torrentUtils;
const LOG_PREFIX = 'RD';
...
const createAbortControllerWithTimeout = debridHelpers.createAbortControllerWithTimeout;
...
function formatCachedResult(torrent, isCached) {
  const episodeHint = torrent.episodeFileHint || null;
    ...
    const normalizedHash = normalizeInfoHash(torrent.InfoHash || torrent.hash || torrent.infoHash);
    ...
    if (torrent.isPersonal) {
        ...
        const baseMagnet = `magnet:?xt=urn:btih:${normalizedHash || torrent.InfoHash || torrent.hash || ''}`;
...
function combineAndMarkResults(apiKey, personalFiles, externalSources, specificSearchKey) {
    ...
    for (const t of externalTorrents) {
        const hash = normalizeInfoHash(t.InfoHash || t.hash || t.infoHash);
        if (!hash) continue;
        ...
        if (!existing || (t.isConfirmedCached && !existing.isConfirmedCached)) {
            uniqueMap.set(hash, { ...t, InfoHash: hash });
        }
    }
    const uniqueExternalTorrents = [...uniqueMap.values()];

    const personalHashes = new Set(personalFiles.map(f => normalizeInfoHash(f.hash)).filter(Boolean));
...
async function searchRealDebridTorrents(apiKey, type, id, userConfig = {}, clientIp = null, isBackgroundRefresh = false) {
  ...
  const { signal, cleanup: cleanupAbort } = createAbortControllerWithTimeout(SEARCH_TIMEOUT_MS, 'RD');
  ...
  try {
    ...
    // OPTIMIZATION: Fetch personal files, scrapers, AND cached results IN PARALLEL for faster results
    // Phase 0 + Phase 1 + Cache lookup now run concurrently instead of sequentially
    const personalFilesPromise = (async () => {
      ...
      let files = await searchPersonalFiles(apiKey, searchKey, 0.3, clientIp);
      ...
      if (type === 'series' && episodeInfo) {
        ...
        if (files.length < originalCount) {
            console.log(`[${LOG_PREFIX}] Filtered personal files for S${episodeInfo.season}E${episodeInfo.episode}: ${originalCount} -> ${files.length}`);
        }
      }
    ...
    // Phase 1: Execute coordinated scrapers IN PARALLEL with personal files fetch
    const scraperResultsPromise = searchCoordinator.executeSearch(
      'realdebrid',
      async () => {
    ...
    // NEW: Check cached scraper results in parallel - these are results from previous background scraper runs
    ...
    // OPTIMIZATION: Await all three promises in parallel - saves 200-500ms by not waiting for personal files first
    const [enrichedPersonalFiles, scraperResults, cachedScraperResults] = await Promise.all([
      personalFilesPromise,
      scraperResultsPromise,
      cachedScraperResultsPromise
    ]);

    console.log(`[${LOG_PREFIX}] Parallel fetch complete: ${enrichedPersonalFiles.length} personal files, scrapers returned, ${cachedScraperResults.length} cached results`);

    // Combine live scraper results with cached scraper results (dedup happens in combineAndMarkResults)
    const allExternalResults = [...scraperResults, cachedScraperResults];
    let combinedResults = combineAndMarkResults(apiKey, enrichedPersonalFiles, allExternalResults, specificSearchKey);
    ...
    // Phase 2: OPTIMIZED - Prioritize high-quality torrents for foreground cache check
    // Instead of checking ALL hashes, we prioritize and check only top torrents to return results faster
    const releaseKey = makeReleaseKey(type, imdbId, episodeInfo?.season, episodeInfo?.episode);
    const allExternalHashes = [...new Set(externalTorrents.map(t => normalizeInfoHash(t.InfoHash || t.hash || t.infoHash)).filter(Boolean))];

    // Sort torrents by priority: 4K/1080p Remux/BluRay first
    const prioritizeTorrents = (torrents) => {
      ...
      return [...torrents].sort((a, b) => {
        const catA = getQualityCategory(a.Title || a.name || '');
        const catB = getQualityCategory(b.Title || b.name || '');
        const resA = getResolutionFromName(a.Title || a.name || '');
        const resB = getResolutionFromName(b.Title || b.name || '');

        const scoreA = (qualityScore[catA] || 0) * 10 + (resolutionScore[resA] || 0);
        const scoreB = (qualityScore[catB] || 0) * 10 + (resolutionScore[resB] || 0);

        return scoreB - scoreA; // Higher score first
      });
    };

    const prioritizedTorrents = prioritizeTorrents(externalTorrents);

    // Check top 50 priority hashes in foreground for better coverage while maintaining speed
    const FOREGROUND_HASH_LIMIT = parseInt(process.env.FOREGROUND_CACHE_LIMIT, 10) || 20;
    const foregroundTorrents = prioritizedTorrents.slice(0, FOREGROUND_HASH_LIMIT);
    const foregroundHashes = [...new Set(foregroundTorrents.map(t => normalizeInfoHash(t.InfoHash || t.hash || t.infoHash)).filter(Boolean))];

    console.log(`[${LOG_PREFIX}] Fast-path: checking ${foregroundHashes.length}/${allExternalHashes.length} priority hashes in foreground`);

    // OPTIMIZATION: Check SQLite and StremThru IN PARALLEL for maximum speed
    // Both checks run concurrently, saving 200-500ms compared to sequential
    ...
    const [sqliteCachedHashes, stremthruCachedHashes] = await Promise.all([
      // SQLite/PostgreSQL check - check ALL hashes (local DB is fast, ~100ms)
      // This ensures background-cached results from previous requests are found immediately
      (async () => {
        const result = new Set();
        if (sqliteCache?.isEnabled()) {
          try {
            // Check ALL hashes against local cache, not just foreground
            const results = await sqliteCache.getCachedHashes('realdebrid', allExternalHashes);
            for (const hash of results) {
              result.add(hash.toLowerCase());
            }
            if (results.size > 0) {
              console.log(`[${LOG_PREFIX}] Local cache found ${results.size}/${allExternalHashes.length} cached hashes`);
            }
          } catch (error) {
      ...
      // StremThru check - only check foreground hashes (external API, limit for speed)
      (async () => {
        const result = new Set();
        if (stremThru.isEnabled() && apiKey) {
          try {
            const stResult = await stremThru.checkInstantAvailability(foregroundHashes, 'realdebrid', apiKey, clientIp, sid);
            for (const hash of stResult) {
              result.add(hash.toLowerCase());
            }
          } catch (error) {
    ...
    console.log(`[${LOG_PREFIX}] Parallel cache check completed in ${cacheCheckDuration}ms: LocalDB=${sqliteCachedHashes.size}/${allExternalHashes.length}, StremThru=${stremthruCachedHashes.size}/${foregroundHashes.length}`)
    ...
    const cachedExternalTorrents = externalTorrents.filter(t => combinedCachedHashes.has((normalizeInfoHash(t.InfoHash || t.hash || t.infoHash) || '').toLowerCase()));
    const uncachedExternalTorrents = externalTorrents.filter(t => !combinedCachedHashes.has((normalizeInfoHash(t.InfoHash || t.hash || t.infoHash) || '').toLowerCase()));
    ...
    if (uncachedExternalTorrents.length > 0) {
      ...
      // Cache foreground results to reuse in background (OPTIMIZATION)
      const foregroundCacheResults = new Map();
      for (const hash of stremthruCachedHashes) {
        foregroundCacheResults.set(hash.toLowerCase(), 'stremthru');
      }
      for (const hash of sqliteCachedHashes) {
        if (!foregroundCacheResults.has(hash.toLowerCase())) {
          foregroundCacheResults.set(hash.toLowerCase(), 'sqlite');
        }
      }
      for (const hash of confirmedCachedHashes) {
        if (!foregroundCacheResults.has(hash.toLowerCase())) {
          foregroundCacheResults.set(hash.toLowerCase(), 'torz');
        }
      }

      console.log(`[${LOG_PREFIX}] Cached ${foregroundCacheResults.size} foreground results for background reuse`);
      ...
      const rdHandler = {
          ...
          checkCachedHashes: async (hashes) => {
              ...
              // OPTIMIZATION: First check foreground cache results (instant lookup, no API calls)
              const uncheckedHashes = [];
              for (const hash of hashes) {
                const normalized = normalizeInfoHash(hash);
                if (!normalized) continue;
                if (foregroundCacheResults.has(normalized)) {
                  cached.add(normalized);
                } else {
                  uncheckedHashes.push(normalized);
                }
              }

              if (cached.size > 0) {
                console.log(`[RD CACHE-CHECK] Reused ${cached.size} foreground cache results (no API calls needed)`);
              }

              if (uncheckedHashes.length === 0) {
                // All hashes were already checked in foreground
                return cached;
              }

              console.log(`[RD CACHE-CHECK] Checking ${uncheckedHashes.length} new hashes not in foreground cache`);

              // OPTIMIZATION: Check SQLite and StremThru in PARALLEL for background checks too
              const [sqliteResults, stremthruResults] = await Promise.all([
                // SQLite check
                (async () => {
                  if (!sqliteCache?.isEnabled()) return [];
                  try {
                    return await sqliteCache.getCachedHashes('realdebrid', uncheckedHashes);
                  } catch {
                    return [];
                  }
                })(),
                ...
                // StremThru check
                (async () => {
                  if (!stremThru?.isEnabled() || !apiKey) return new Set();
                  try {
                    // Build SId for episode-specific filtering (like Torz does: imdbid:season:episode)
                    let sid = null;
                    if (type === 'series' && imdbId && season && episode) {
                    ...
                    return await stremThru.checkInstantAvailability(uncheckedHashes, 'realdebrid', apiKey, clientIp, sid);
                  } catch (stremError) {
              ...
              for (const hash of sqliteResults) {
                ...
                foregroundCacheResults.set(hashLower, 'sqlite-bg');
              }

              // Process StremThru results
              for (const hash of stremthruResults) {
                const hashLower = hash.toLowerCase();
                cached.add(hashLower);
                foregroundCacheResults.set(hashLower, 'stremthru-bg');
              }
          ...
          liveCheckHash: async (hash) => {
              ...
              try {
                  ...
                  if (!normalizedHash) {
                    console.log(`[${LOG_PREFIX} CACHE-CHECK] Skipping invalid hash: ${hash}`);
                    return false;
                  }
                  ...
                  try {
                    ...
                    await addHashToSqlite(normalizedHash, largestVideo?.path || null, largestVideo?.bytes || null, { status });
                  } catch {}
                  return true;
              } catch (e) {
          ...
          batchCheckSeasonPacks: async (hashes, season, episode) => {
              ...
              for (const hash of hashes) {
                  ...
                  try {
                      ...
                      const existingTorrent = torrentsList.find(t => t.hash && t.hash.toLowerCase() === normalizedHash);
                      ...
                      if (matchingFiles.length > 0) {
                          ...
                          const episodeResult = {
                              InfoHash: normalizedHash, Title: bestFile.path, name: bestFile.path, Size: bestFile.bytes,
                              ...
                              isCached: true, isFromPack: true, packHash: normalizedHash, searchableName: info.data.filename
                          };
                          packResults.set(normalizedHash, [episodeResult]);
                          inspectedCount++;
                      }
                  } catch (error) {
            ...
            if (backgroundCachedResults.length > 0) {
              try {
                if (sqliteCache?.isEnabled()) {
                  ...
                  const uniqueUpsertsToSave = uniqueUpserts(upserts);
                  if (uniqueUpsertsToSave.length > 0) {
                    deferSqliteUpserts(uniqueUpsertsToSave);
                    console.log(`[${LOG_PREFIX}] Background: Deferred ${uniqueUpsertsToSave.length} cached torrents to cache`);
                  }
                }
              } catch (error) {
  ...
  try {
    ...
    if (!torrentId) {
      ...
      try {
        const torrentsResponse = await rdCall(() => RD.torrents.get(0, 1, 100), apiKey);
        const torrentsList = Array.isArray(torrentsResponse?.data) ? torrentsResponse.data : [];
        const hit = torrentsList.find(t => t.hash && t.hash.toLowerCase() === hash && ['downloaded', 'finished', 'uploading'].includes(t.status));
        if (hit) {
          torrentId = hit.id;
...
async function getAllDownloads(RD, apiKey) {
  try {
    ...
    const allDownloads = Array.isArray(response?.data) ? response.data : [];
